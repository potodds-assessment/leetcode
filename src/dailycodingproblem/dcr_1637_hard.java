...
public class dcr_1637_hard {
    ...
    public static int findIdxSorted(int[] arr, int x) {
        // Pre-calculate powers of two for efficient searching
        List<Integer> powersOfTwo = new ArrayList<>();
        powersOfTwo.add(1);
        while (powersOfTwo.get(powersOfTwo.size() - 1) < arr.length) {
            powersOfTwo.add(powersOfTwo.get(powersOfTwo.size() - 1) * 2);
        }

        int idx = 0;
        for (int pot = powersOfTwo.size() - 1; pot >= 0; pot--) {
            if (idx + powersOfTwo.get(pot) < arr.length && x >= arr[idx + powersOfTwo.get(pot)]) {
                idx += powersOfTwo.get(pot);
            }
        }
    ...
    public static void main(String[] args) {
        dcr_1637_hard app = new dcr_1637_hard();
//        System.out.println(app.divideTwo(98));

        int[] arr = {1, 3, 5, 7, 9};
        int x = 100;
        int idx = findIdxSorted(arr, x);
